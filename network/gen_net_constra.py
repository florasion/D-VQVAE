...
from network.contra_net import Contra_Net
class GenNet(nn.Module):
    ...
    def gen(self,obj):
        ...
        while p:
            ...
            idx4 = x_tilde[:,2,1]
            idx5 = x_tilde[:,2,2]
            id_values = [idx_o,idx0, idx1, idx2, idx3, idx4, idx5]
            data_values = self.data
            #print(data_values[0].size)
            for i in range(len(id_values)):
                #print(data_values[0].size)
                if data_values[i][id_values[i].cpu()] == 0:
                    self.num+=1
                    p=True
                    #print() 
                else:
        ...
        recon_mano =  self.rh_mano(betas=recon[:, :10], global_orient=zero_params,
                                hand_pose=recon[:, 10:55], transl=zero_params)  # [B,778,3]
        joint = recon_mano.joints
        recon_,_  = self.Contra_Net(joint,recon[:, 10:55])
        recon[:, 10:55] = recon_
        recon_mano =  self.rh_mano(betas=recon[:, :10], global_orient=zero_params,
        ...
        return recon,recon_pos
    def set_co(self,model):
        self.Contra_Net=model
