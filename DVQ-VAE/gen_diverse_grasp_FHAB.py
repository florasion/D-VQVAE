...
from dataset.dataset_FHAB import FHAB_diversity
...
def main(args, model, eval_loader, device, rh_mano, rh_faces):
    ...
    for batch_idx, (obj_pc, origin_verts, origin_faces) in enumerate(eval_loader):
        ...
        for i in range(49):
            ...
            mean_xyz_obj = torch.tensor(origin_verts).mean(dim=(0))
            trans = np.array([-0.0793, 0.0208, -0.6924]) - np.array(mean_xyz_obj)+ np.random.random(3) * 0.2 *0
            ...
            for j in range(0,0):  # non-learning based optimization steps
                optimizer.zero_grad()
                ...
                penetr_loss, contact_loss = TTT_loss(recon_xyz, rh_faces,
                                                                       obj_pc_TTT[:, :3, :].permute(0,2,1).contiguous(),
                                                                       cmap_affordance)
                loss = 1 * contact_loss + 7 * penetr_loss
                loss.backward()
                optimizer.step()
                if j == 0 or j == 299:
                    print("Object sample {}, pose {}, iter {}, "
                          ...
                          "contact loss {:9.5f}".format(batch_idx, i, j,
                                                        penetr_loss.item(),  contact_loss.item()))
            ...
            if sample_contact:
                total_contact+=1
            print('sample num:{}, mean penetr vol:{}, mean simu disp:{}, contact ratio:{},mean_time:{}'
                  .format(total_num,total_penetr_vol/total_num,total_simu_disp/total_num,total_contact/total_num,total_time/total_num))
            if True:
        ...
        save_path = './diverse_grasp/FHAB/obj_id_{}.json'.format(batch_idx)
        print(save_path)
        data = {
