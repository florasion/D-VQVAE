...
def main(args, model, eval_loader, device, rh_mano, rh_faces):
    ...
    for batch_idx, (obj_id, obj_pc, origin_verts, origin_faces) in enumerate(eval_loader):
        try:
            assert obj_id in [3, 4, 6, 10, 11, 19, 21, 25, 35, 37]
        except:
            continue
        ...
        for i in range(100):
            ...
            try:
                simu_disp = run_simulation(final_mano_verts, rh_faces.reshape((-1, 3)),
                                          ...
                                          vhacd_exe=vhacd_exe,conn_id=None,save_video=False)
                #print('run success')
            except:
                simu_disp = 0.10
            '''
            obj_xyz_rotate=rotate_point_cloud(obj_mesh_verts)
            recon_xyz_rotate=rotate_point_cloud(final_mano_verts)
            simu_disp_rotate = run_simulation(recon_xyz_rotate, rh_faces.reshape((-1, 3)),
                                                      obj_xyz_rotate, origin_faces.cpu().numpy().astype(np.int32).reshape((-1, 3)),
                                                      vhacd_exe=vhacd_exe, conn_id=None,save_video=False)
            simu_disp =(simu_disp+ simu_disp_rotate)/2
            '''
            print('simu disp:',simu_disp)
            ...
            with open('./logs/ho3d.txt', 'a') as file:
                    file.write('sample num:{}, mean penetr vol:{}, simu disp:{}, mesh_dist:{}\n'
                            .format(total_num,vol,simu_disp,mesh_dist))
